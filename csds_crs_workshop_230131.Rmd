...
date: "Tuesday, 31 January 2023, presented in earlier version on Thursday 19 January, 2023, 10:00-11:30 CT"
output:
  html_document:
    toc: true
    toc_float:
...
_Warning: January 19 version goes astray here:_ the local default user-writable directory containing the cache file was then not empty:
...
```{r}
library(RSQLite)
if (length(list.files(pths[1])) > 0L) {
db <- dbConnect(SQLite(), dbname=file.path(pths[1], "cache.db"))
unique(dbReadTable(db, "chunks")$url)
dbDisconnect(db)
}
```

_So the presentation of January 19 started using `us_noaa_conus.tif` because it was available in the current working directory for a figure below and in the local default user-writable directory, but did not obtain the full accuracy that the NAD83 (HARN) grid made available._
...
These discrepancies stem from displacement when digitising in the early 1990s, but the relative positions of the tracts are preserved.
...
cat(o, sep="\n")
```
This corresponds to the result above using horizontal grid shift.
