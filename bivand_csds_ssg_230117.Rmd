...
- The maintainer of **raster**, Rober Hijmans, has already replaced calls to **rgdal** and **rgeos** in **raster** to calls to **terra**
...
- The plan is to retire **maptools**, **rgdal** and **rgeos** during 2023 while changing **sp** to use **sf** under the hood, so that the package remains active; this has already been doone for **raster** with use of retiring packages now replaced by calls to **terra**
...
- Within four weeks and over the holiday period, activity was observed for 33\% of the packages (32\% of those using retiring packages in code)

- 43\% of the github issues showed activity, but only 12\% of the emails elicited any response (only one bounced, but false positive spam detection is not unlikely)
